        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`relative flex flex-col items-center gap-1 p-3 rounded-xl transition-all duration-300 z-10 flex-1 ${
                isActive 
                  ? 'text-white transform scale-105' 
                  : 'text-muted-foreground hover:text-primary hover:scale-105'
              }`}
            >
              <Icon className={`w-4 h-4 transition-all duration-500 ${isActive ? 'animate-bounce-gentle' : ''}`} />
